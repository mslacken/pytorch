...
//     auto rnn_output = m->forward_with_packed_input(packed_input, torch::Tensor());
//     auto expected_output = torch::tensor(
//     {{-0.0645, -0.7274, 0.4531},
//      {-0.3970, -0.6950, 0.6009},
//      {-0.3877, -0.7310, 0.6806}}); // yf225 TODO: check Python equivalent output
//     std::cout << std::get<0>(rnn_output).data() << std::endl;
...
//     auto m = LSTM(2, 3);
//     torch::nn::utils::rnn::PackedSequence packed_input = torch::nn::utils::rnn::pack_sequence({torch::ones({3, 2})});
//     auto rnn_output = m->forward_with_packed_input(packed_input, torch::nullopt);
//     auto expected_output = torch::tensor(
...
//       {-0.3889, -0.1919,  0.1183},
//       {-0.3889, -0.1919,  0.1183}}}); // yf225 TODO: check Python equivalent output
//     std::cout << std::get<0>(rnn_output).data() << std::endl;
//     ASSERT_TRUE(torch::allclose(std::get<0>(rnn_output).data(), expected_output, 1e-05, 2e-04));
//   }
//   {
...
//       {-0.1189,  0.0502,  0.2960},
//       {-0.1189,  0.0502,  0.2960}}}); // yf225 TODO: check Python equivalent output
//     std::cout << std::get<0>(rnn_output).data() << std::endl;
//     ASSERT_TRUE(torch::allclose(std::get<0>(rnn_output).data(), expected_output, 1e-05, 2e-04));
//   }
// }
